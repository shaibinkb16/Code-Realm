...
export const SocialAuthButtons: React.FC<SocialAuthButtonsProps> = ({ onSuccess, onError }) => {
  ...
  const handlePasskeyLogin = async () => {
    if (!passkeyService.isSupported()) {
      onError("Passkeys are not supported on this device/browser.");
      return;
    }

    setIsPasskeyLoading(true);
    try {
      const tokenData = await passkeyService.loginWithPasskey();
      onSuccess(tokenData);
    } catch (err: any) {
      onError(err.message || 'Passkey verification failed');
    } finally {
